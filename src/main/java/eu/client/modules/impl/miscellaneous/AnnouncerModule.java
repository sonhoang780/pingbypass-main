...
public class AnnouncerModule extends Module {
    public StringSetting watermark = new StringSetting("Watermark", "The client name that will be used in the announcer.", EUClient.MOD_NAME);
    ...
    public void onTick(TickEvent event) {
        ...
        if(lastPos == null) lastPos = new Vec3(mc.player.xo, mc.player.yo, mc.player.zo);
        double traveled = Math.abs(lastPos.x - mc.player.xo) + Math.abs(lastPos.y - mc.player.yo) + Math.abs(lastPos.z - mc.player.zo);
        ...
        if(distance.getValue() && traveled > 1 && distanceTimer.hasTimeElapsed(10000) && queue.size() <= 5) {
            ...
            lastPos = new Vec3(mc.player.xo, mc.player.yo, mc.player.zo);
            distanceTimer.reset();
        }
    }
    ...
    public void onPacketSend(PacketSendEvent event) {
        ...
        if(event.getPacket() instanceof ServerboundUseItemOnPacket packet && mc.player.getItemInHand(packet.getHand()).getItem() instanceof BlockItem) {
            placed++;
    ...
    public void onConsumeItem(ConsumeItemEvent event) {
        if(getNull() || event.getStack().get(DataComponents.FOOD) == null) return;

        eaten++;

        if(eating.getValue() && eaten >= MathUtils.random(6, 1) && queue.size() <= 5) {
            queue.add(getEatMessage(eaten + " " + event.getStack().getHoverName().getString()));
            eaten = 0;
        }
    }
