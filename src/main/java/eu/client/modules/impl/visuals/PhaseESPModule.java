...
public class PhaseESPModule extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        for(Direction direction : Direction.values()) {
            ...
            if(mc.level.getBlockState(offsetPos).getBlock() != Blocks.BEDROCK && mc.level.getBlockState(offsetPos).getBlock() != Blocks.OBSIDIAN) continue;
    ...
    public void onRenderWorld(RenderWorldEvent event) {
        ...
        for(PhaseBlock phaseBlock : phaseBlocks) {
            AABB filledBox = new AABB(phaseBlock.box().minX, phaseBlock.box().minY, phaseBlock.box().minZ, phaseBlock.box().maxX, phaseBlock.box().minY + fillHeight.getValue().doubleValue(), phaseBlock.box().maxZ);
            AABB outlinedBox = new AABB(phaseBlock.box().minX, phaseBlock.box().minY, phaseBlock.box().minZ, phaseBlock.box().maxX, phaseBlock.box().minY + outlineHeight.getValue().doubleValue(), phaseBlock.box().maxZ);
    ...
    private PhaseType getPhaseType(BlockPos pos) {
        if(mc.level.getBlockState(pos.below()).getBlock() == Blocks.BEDROCK && bedrock.getValue()) return PhaseType.BEDROCK;
        if(mc.level.getBlockState(pos.below()).getBlock() == Blocks.OBSIDIAN && obsidian.getValue()) return PhaseType.OBSIDIAN;
        if(mc.level.getBlockState(pos.below()).getBlock() == Blocks.AIR && air.getValue()) return PhaseType.AIR;
        return null;
    }
