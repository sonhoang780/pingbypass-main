...
public class HoleFillModule extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        Runnable runnable = () -> {
            ...
            boolean flag = webs.getValue() ? !mc.player.getMainHandItem().getItem().equals(Items.COBWEB) : !(mc.player.getMainHandItem().getItem() instanceof BlockItem);
    ...
    private List<BlockPos> getPositions(Player player) {
        ...
        for (int i = 0; i < EUClient.WORLD_MANAGER.getRadius(range.getValue().doubleValue()); i++) {
            ...
            if(webs.getValue() && selfWeb.getValue() && PositionUtils.getFlooredPosition(mc.player).equals(position) && HoleUtils.isPlayerInHole(mc.player)
                    && player.getY() > mc.player.getY() && mc.player.distanceTo(player) <= 2 && mc.player.onGround()) {
