...
public class WhitelistButton extends Button {
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        Renderer2D.renderQuad(context, getX() + getPadding() + 1, getY(), getX() + getWidth() - getPadding() - 1, getY() + 13 - 1, ClickGuiScreen.getButtonColor(getY(), 100));
        Renderer2D.renderQuad(context, getX() + getPadding() + 1, getY(), getX() + getPadding() + 2, getY() + getHeight() - 1, ClickGuiScreen.getButtonColor(getY(), 30));

        EUClient.FONT_MANAGER.drawTextWithShadow(context, setting.getTag(), getX() + getTextPadding() + 1, getY() + 2, Color.WHITE);
        EUClient.FONT_MANAGER.drawTextWithShadow(context, ChatFormatting.GRAY + "" + setting.getWhitelist().size(), getX() + getWidth() - getTextPadding() - 1 - EUClient.FONT_MANAGER.getWidth(setting.getWhitelist().size() + ""), getY() + 2, Color.WHITE);

        if (open) {
            int contentY = getY() + getParent().getHeight();

            Renderer2D.renderQuad(context, getX() + getPadding() + 1 + offsetX, contentY, getX() + getWidth() - getPadding() - 1, contentY + searchBarHeight, ClickGuiScreen.getButtonColor(getY(), 30));
            ...
            for (int i = 0; i < renderCount; i++) {
                ...
                Renderer2D.renderQuad(context, getX() + getPadding() + 1 + offsetX, itemY, getX() + getWidth() - getPadding() - 1, itemY + elementHeight, ClickGuiScreen.getButtonColor(getY(), 30));
                ...
                for (ColoredSegment seg : segments) {
                    textWidth += EUClient.FONT_MANAGER.getWidth(seg.text);
                }
                ...
                for (ColoredSegment seg : drawSegments) {
                    EUClient.FONT_MANAGER.drawTextWithShadow(context, seg.text, drawX, itemY + 2, seg.color);
                    drawX += EUClient.FONT_MANAGER.getWidth(seg.text);
                }
                ...
                EUClient.FONT_MANAGER.drawTextWithShadow(context, symbol, symbolRenderX, itemY + 2, symbolColor);
            }
            ...
            if (visibleElements.size() > maxDisplayed) {
                ...
                Renderer2D.renderQuad(context, scrollbarX, listStartY, scrollbarX + scrollbarWidth, listStartY + scrollbarFullHeight, ClickGuiScreen.getButtonColor(getY(), 40));
                Renderer2D.renderQuad(context, scrollbarX, listStartY + scrollbarPos, scrollbarX + scrollbarWidth, listStartY + scrollbarPos + barHeight, ClickGuiScreen.getButtonColor(getY(), 200));
            }
        }
    }
    ...
    public void keyPressed(int keyCode, int scanCode, int modifiers) {
        if (!searching) return;
        com.mojang.blaze3d.platform.Window handle = mc.getWindow();

        boolean ctrlPressed = InputConstants.isKeyDown(handle, (net.minecraft.util.Util.getPlatform() == net.minecraft.util.Util.OS.OSX) ? GLFW.GLFW_KEY_LEFT_SUPER : GLFW.GLFW_KEY_LEFT_CONTROL);
        ...
        if (keyCode == GLFW.GLFW_KEY_ESCAPE) {
            ...
            if (keyCode == GLFW.GLFW_KEY_V) {
                try {
                    ...
                    EUClient.LOGGER.error("{}: Failed to process clipboard paste", e.getClass().getName(), e);
                }
            } else if (keyCode == GLFW.GLFW_KEY_C && selecting) {
                try {
                    mc.keyboardHandler.setClipboard(searchQuery);
                } catch (Exception e) {
                    EUClient.LOGGER.error("{}: Failed to process clipboard change", e.getClass().getName(), e);
                }
    ...
    private List<ColoredSegment> truncateSegments(List<ColoredSegment> segments, int maxWidth) {
        ...
        for (ColoredSegment seg : segments) {
            int segWidth = EUClient.FONT_MANAGER.getWidth(seg.text);
            if (currentWidth + segWidth <= maxWidth) {
                ...
                if (spaceLeft > 0) {
                    ...
                    for (int i = 0; i < seg.text.length(); i++) {
                        ...
                        int charWidth = EUClient.FONT_MANAGER.getWidth(String.valueOf(c));
                        if (charWidth <= spaceLeft - EUClient.FONT_MANAGER.getWidth("…")) {
                            sb.append(c);
                            spaceLeft -= charWidth;
                        } else {
                            break;
