...
target_block = '''    private boolean executeBasePlace() {
        ...
        if (this.placeTarget != null && this.placeTarget.getPosition() != null) return false;
        ...
        if (existingPlacement != null && existingPlacement.getPosition() != null) {
            ...
            // Only suppress BasePlace if the placement is AT FOOT LEVEL (adjacent to targetFeet.below())
            // and deals valid damage. If the placement is down on a bedrock floor under an elevated pillar,
            // it is blocked by the pillar and does not provide an effective foot-level attack, so BasePlace must still run!
    ...
    private boolean hasAdjacentCrystalBase(BlockPos block1) {
        if (mc.level == null) return false;
        for (Direction dir : Direction.Plane.HORIZONTAL) {
            BlockPos adj = block1.relative(dir);
            BlockState state = mc.level.getBlockState(adj);
            if (state.getBlock() == Blocks.OBSIDIAN || state.getBlock() == Blocks.BEDROCK) {
                BlockPos crystalSpace = adj.above();
                ...
                if ((spaceState.isAir() || spaceState.canBeReplaced()) && mc.level.getFluidState(crystalSpace).isEmpty()) {
                    AABB crystalBox = new AABB(crystalSpace.getX(), crystalSpace.getY(), crystalSpace.getZ(),
                            crystalSpace.getX() + 1.0, crystalSpace.getY() + 2.0, crystalSpace.getZ() + 1.0);
                    List<Entity> collisions = mc.level.getEntities((Entity) null, crystalBox, e -> !(e instanceof EndCrystal) && !(e instanceof ItemEntity) && !(e instanceof ExperienceOrb));
...
replacement_block = '''    private boolean executeBasePlace() {
    ...
    private boolean isBaseCandidateValid(BlockPos pos) {
        ...
        return mc.level.getEntities((Entity) null, new AABB(crystalSpace), entity -> entity.isAlive() && !(entity instanceof ExperienceOrb) && !(entity instanceof EndCrystal)).isEmpty();
    }'''
